...
function Create() {
    ...
    //         .then(setCategory("todos")
    //         ,setNotes(current =>([...current,{title:newNote.title,details:newNote.details,category:category, id: [current[notes.length -1 ].id + 1] || 1}])),
    //         setNewNote({title:"",details: null, error: false})
        ...
        event.preventDefault()
           try{     console.log("submitting task")
            fetch(`https://note-backend-zachary-9a350c884dc1.herokuapp.com/tasks/create`, {
                ...
                body: JSON.stringify({title:newNote.title, details: newNote.details, category: category, completion: newNote.completed, due: newNote.due, time:newNote.time}),
                })
                .then(res=> res.json())
